...
const Layout = ({ children }) => {
  ...
  return (
    <Background>
      ...
      <div
        className={`duration-400 fixed top-0 z-20 h-screen transition-all ease-in-out ${
          isLeftbarVisible
            ? 'w-[15rem] translate-x-0'
            : 'w-[2rem] -translate-x-[13rem] opacity-0'
        }`}
        onMouseEnter={() => setIsLeftbarVisible(true)} // 마우스 호버 시 Leftbar 표시
        onMouseLeave={() => setIsLeftbarVisible(false)} // 마우스 떠날 때 Leftbar 숨김
      >
        <div className="absolute h-full transition-transform duration-300 ease-in-out">
